...
`SerialInteractivePromptQueue` owns FIFO prompt admission, combines session and request-local cancellation, and keeps later prompts behind an active transaction until its cleanup settles.
...
When `ask`-state permissions arise in a headless subagent or explicitly delegated interactive child, the extension forwards the dialog to the parent session rather than silently denying.
Interactive delegation takes precedence over UI presence and ambient parent hints, requires a live `interactive-delegation-v1` handshake, and blocks before hooks while unready.
`permission-delegation.ts` owns the immutable binding and observation-only wait set; `delegation-control.ts` negotiates exact identities, maintains liveness, and revokes retired bindings.
`forwarded-transaction.ts` validates negotiated envelopes and arbitrates cancellation versus response with one atomic publish-if-absent terminal.
The winning publication is the commit point; grant application follows synchronously, and post-commit cancellation does not revoke a valid serving grant.
The legacy headless path remains available without downgrading a required delegate.
The child stamps one absolute deadline when it creates the request, and both sides enforce it before accepting a response, recording a grant, or emitting a terminal approval.
The serving side cancels queued or active UI work at that deadline; direct prompts have no forwarding deadline.
A deadline expiry is automatic `confirmation_unavailable` and cannot be replaced by a late human result.
Legacy exchanges produce no timeout response file; negotiated exchanges retain an immutable unavailable terminal.
Legacy requests without the additive deadline field retain their former serving behavior.
...
At `session_start`, the extension publishes every exact-session `PermissionsService` via `publishPermissionsService()`; registered children and required delegates do not replace the default parent selection.
Other extensions import `getPermissionsService` from `@gotgenes/pi-permission-system` and resolve `getPermissionsService(sessionId)` for each query; the no-argument overload is reserved for the parent/default service.
Registrations reconcile from both the consumer's startup and `permissions:ready`, disposing old registrations on replacement/shutdown; publication does not imply delegation readiness.
...
The `PermissionsService` interface exposes query/registration methods plus delegation lifecycle operations:
...
- `connectDelegation(identity, options?)` - establish a required child's exact live parent binding.
- `getDelegationState()` - immutable lifecycle snapshot, not a tool approval.
- `subscribeDelegatedWaits(listener)` - immediate and subsequent immutable pending-set observations; returns an unsubscribe-only disposer.
...
1. **`LocalUserAuthorizer`** — the nondelegated session has UI; prompt the human here.
2. **`ParentAuthorizer`** — a required interactive child has a ready binding, or a legacy headless child has a parent target; escalate to the serving parent.
...
Selection refreshes on session activation, with explicit required delegation before the legacy `hasUI`/`isSubagent` branches.
An unready required child never selects local UI, and ordinary same-session activation preserves its binding.
...
│       ├── descriptor.ts     GateDescriptor (carrying the `PromptPayload` as its single presentation fact), GateBypass, GateResult types, plus `DecisionEventFacts` (a decision event minus the `requestId` only the runner can supply). Constraint: `promptDetails` omits both `requestId` and `payload`, which the runner stamps, so a gate cannot supply either twice
│       ├── pre-tool-use-hook-gate.ts `PreToolUseHookGate` - production first gate; runs configured hooks with session context, records abnormal execution diagnostics, and checks the captured request lifetime before reporting a verdict; active allow/deny terminates, while ask/defer continues to the dialog fallback
...
├── service-lifecycle.ts      Retires the prior same-session owner before serving activation; publishes exact-session services, guards the parent/default selection, emits local service readiness, and tears down by identity
├── service.ts                PermissionsService and delegation/wait types; one Symbol.for()-backed service map keyed by exact session IDs and a distinct Symbol legacy-default key; public surface bundled in dist/public.d.ts
...
│   ├── agent-renderer.ts     `EXTENSION_TAG` + `renderPolicyDenial`/`renderUserDenial`/`renderUnavailableDenial`/`renderForwardingTimeoutDenial` — the agent-facing render of a refused ask. Constraint: it identifies the call and never reproduces it (ADR 0011 §7) — the bash command is never rendered, and the flagged path/target/skill is capped
...
│   ├── permission-delegation.ts Required-marker validation, exact-identity binding lifecycle, runtime routing capability, and immutable observation-only pending-wait snapshots
│   ├── delegation-control.ts Capability handshake, in-memory admissions, and idempotent reuse of a matching accepted ACK for a live same-instance binding; replacement instances still reject consumed-request replay. Child liveness and persistent binding revocations use a separate control root
│   ├── forwarded-transaction.ts Strict correlated terminal records and publish-if-absent cancellation/decision arbitration; timeout-aware unavailable results and abort-aware waiting
...
│   ├── local-user-authorizer.ts `LocalUserAuthorizer` class - `TerminalAuthorizer` for a session with UI and the single `permissions:ui_prompt` emit site: brackets the locally served human prompt with the injected `setPromptIndicator`, rejects an expired forwarded deadline before UI or broadcast work, renders a live forwarded ask's provenance as a non-degraded broadcast + `(Subagent)` title, then dispatches to the inline keybind dialog (TUI) or the `select`/`input` fallback
│   ├── interactive-prompt-queue.ts `SerialInteractivePromptQueue` - FIFO admission for complete prompt transactions; combines generation-wide invalidation with optional request-local cancellation while retaining the underlying interaction as the ordering barrier until cleanup settles, shared across replacement instances of the same session
│   ├── permission-dialog.ts   Dialog option semantics + deadline-aware `requestPermissionDecisionFromUi` (`select`/`input` fallback with remaining SDK timeout) + `PermissionPromptDecision` (whose `decidedBy` is required and whose runtime-only `forwardingTimedOut` marker identifies deadline expiry) and `UnattributedDecision` (the same minus it); the mode dispatch lives in `permission-prompt-component.ts`
...
│   ├── permission-prompter.ts `PermissionPrompter` class (`PermissionPrompterApi`) - review-log bracketing (waiting → approved/denied) around `authorizer.authorize(details)`, recording the decision's `decidedBy` on the outcome entries only (the waiting entry has no decider yet); `PromptPermissionDetails` type (carries the child-fixed `accessIntent` facts a forwarded ask relays plus its inseparable request-local deadline signal/timestamp pair)
...
│   ├── forwarding-liveness.ts The filesystem half of the same question, for a child that shares no memory with its parent: `ServingHeartbeatStore` (a `ServingAnnouncer` publishing `<forwardingDir>/serving/<id>.json` with the served session, its pid, and its refresh time; throttled, never throws, and sweeps records of dead processes once per session) + `HeartbeatReader` classifying a target as alive/absent/stale/dead_pid only after the shared reader validates the embedded identity against its canonical record path + `ForwardingLivenessJudge` (`TargetServingLookup`), which routes a liveness question to the channel that can answer it by the target's `self`/`registry`/`env` provenance. Constraint: the records live beside `sessions/`, never inside it, so liveness stays disjoint from the request/response cleanup ordering (#398)
...
│   ├── permission-forwarding.ts Cross-session forwarding wire types (`ForwardedPermissionRequest`, which carries the child's `PromptPayload` and optional absolute `expiresAt` rather than a sentence assembled under the child's config; `ForwardedPermissionResponse`, whose optional `decidedBy` names what decided inside the responding session, distinct from the `responderSessionId` that names where; the `ForwardedAccessFacts`/`ForwardedAccessIntent` intent schema per ADR 0008) + `resolvePermissionForwardingTarget`, which returns the resolved session id together with its `self`/`registry`/`env` provenance (the routing key for which liveness channel may judge the target) + `encodeSessionIdForPath`, shared by session-keyed paths under the forwarding root; encodes whole-segment `.`/`..` as `%2E`/`%2E%2E` so `path.join` cannot collapse a session namespace
│   ├── approval-escalator.ts  `ParentAuthorizer` class - `TerminalAuthorizer` for a subagent session: escalates the ask up the tree via the request-write/poll machinery, completing the child-fixed facts into a `ForwardedAccessIntent` (stamps `requesterCwd`/`principal`), `ctx` bound at construction; adopts the requester's `requestId` as the forwarded request's `id` (falling back to a fresh mint when it could not safely name a file — at a relay hop that id came off disk); a negotiated request disappearing without a terminal triggers a correlated re-read and atomic unavailable-versus-approval settlement, clearing only that wait while retaining a healthy binding; stamps one deadline from the configured timeout snapshot, polls against that same timestamp, and makes deadline expiry the shared timeout denial; every other abandonment path (unresolvable target, unusable directories, unwritable request, unserved target, unreadable response) denies with `confirmationUnavailable` plus a path-naming `denialReason` — reused verbatim as the `unavailable` decider's reason so the two cannot drift — and discards the request so a late answer cannot arrive; an answered request's decision is nested under a `forwarded` decider carrying the responder's own
│   ├── forwarded-request-server.ts `ForwardedRequestServer` class (`InboxProcessor`) - serving-down role: `processInbox()` correlates every legacy/delegated request ID with its inbox filename before resolving it like a local action - `ServingPolicy` (recorded authority) then `AskEscalator` on `ask`; `ServingPolicy.resolve(intent: ForwardedAccessIntent)` is intent-shaped (agent-scoped to `principal.agentName`, child-fixed `matchValues` used as-is, never re-derived through this session's `PathNormalizer`/cwd), floors to `ask` when `accessIntent` is absent (version skew); projects the request's access facts onto the escalated ask (`surface`/`matchValues`/`boundaryValue` only — `requesterCwd`/`principal` stay off the ask details, and the bounded-delegation checkpoint's exclusion reads the projected gate surface, #635); enforces a new request's absolute deadline before policy, escalation, grant, event, and response boundaries; writes no legacy response on timeout and ignores late non-cooperating escalation results; negotiated requests instead commit a strict terminal before applying any serving grant; otherwise writes its decider onto the response (its own matched rule in full, the escalated decision's source, or a `gate_error` when the escalation itself threw), and the grant-scope translation rewrites the scope but never the decider; broadcasts the terminal `permissions:decision` for every ask it escalates, rendered from the same `PromptPermissionDetails` its `permissions:ui_prompt` was built from, so a prompt the requesting session's gate would answer on another bus is clearable on this one — a recorded-authority resolution stays silent on both channels; one-hop canary
│   ├── forwarding-io.ts       Forwarding filesystem helpers - request/response read-write (nonempty request IDs without separators or NUL; strict safe-integer/order validation for request `createdAt` and present `expiresAt`; tolerant read of the optional `accessIntent` and `decidedBy` fields; an unusable decider is dropped without rejecting the decision it accompanies), location derivation, atomic JSON writes (owner-only; `rename` preserves the temp file's mode). Constraint: the readers rebuild an allowlist of known fields, so a wire field added without being listed here is silently dropped
│   └── forwarding-manager.ts  `ForwardingController` interface + `ForwardingManager` class - drives the forwarded-permission inbox polling lifecycle; tells `ForwardedRequestServer.processInbox`, and publishes the polled session id to the `ServingAnnouncer` plus a `forwarded_permission.serving_started`/`serving_stopped` review entry. Constraint: heartbeat refresh, handshake processing, and pending-request cancellation checks run ahead of the busy guard; drain ownership prevents retired work from resetting a replacement's busy flag; required delegates never serve
...
├── zellij-tab-alert.ts        `ZellijTabAlert` class - opt-in serving-tab name prefix and Pi-pane background while one locally served human prompt is active; preserves manual tab renames, resets the pane independently, and resolves every expected command/discovery failure after debug logging
