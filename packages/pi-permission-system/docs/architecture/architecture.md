...
A deadline expiry is automatic `confirmation_unavailable`, produces no timeout response file, and cannot be replaced by a late human result.
...
│   ├── interactive-prompt-queue.ts `SerialInteractivePromptQueue` - FIFO admission for complete prompt transactions; combines generation-wide invalidation with optional request-local cancellation while retaining the underlying interaction as the ordering barrier until cleanup settles
