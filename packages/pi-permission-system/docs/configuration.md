...
The inherited deterministic policy engine remains in the package for compatibility, diagnostics, and migration.
...
Project config is loaded only after Pi reports that the project is trusted.
...
Pi tool names are translated to Claude Code-compatible names before matcher evaluation:
...
Unknown extension tool names are passed through unchanged.
...
The `if` condition supports common command and path inputs:
...
Hooks therefore require a POSIX-compatible `sh` on `PATH`; stock Windows without Git Bash, MSYS2, or another compatible shell records `spawn_error` and defers to the dialog.
`transcript_path` is the active session JSONL file and is omitted when Pi does not expose one.
...
Multiple hook results are merged as `deny > ask > allow > defer`.
Reasons from results with the winning decision are retained in the terminal message.
`updatedInput` and `additionalContext` are recognized but ignored because this extension only owns authorization, not tool-input rewriting or model-context mutation.
...
Maximum time in milliseconds a subagent waits for a serving parent session to answer a forwarded ask.
An in-process target known not to be serving fails earlier regardless of this value.

Default: `600000`.

### Preview limits

`toolInputPreviewMaxLength` limits the inline JSON preview shown in permission prompts.
`toolTextSummaryMaxLength` limits compact command, pattern, and path summaries.

### Logging fields

`permissionReviewLog` enables structured request and decision audit events.
...
A hook denial therefore remains authoritative even when the user previously approved a matching session pattern.

## Subagent Behavior

A child session runs its own hook before forwarding.
The hook input carries the child's session ID, working directory, transcript path, tool name, and raw tool input.
When the child hook asks or defers, the final dialog request is forwarded to the serving parent session.
The parent does not run an `authorizerChain` over that request.

A forwarded session grant can apply either to the requesting child or to the whole serving session.

## Compatibility Fields

The following fields remain accepted so existing config files do not fail strict validation during migration:
...
Remove these fields after migrating their intended approvals or denials into the hook implementation.
...
A terminal hook decision emits a `permissions:decision` event with resolution `hook_approved` or `hook_denied` and origin `pretooluse_hook`.
Hook asks and deferrals do not emit a terminal decision because the session approval or dialog owns the eventual result.
